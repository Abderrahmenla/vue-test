<template>
  <div
    class="bg-white flex items-center justify-center px-4 pt-3 pb-8 rounded-2xl shadow-sm border border-gray-300 w-full mx-auto"
  >
    <div
      v-for="(dayData, index) in weekData"
      :key="index"
      :class="['flex items-center justify-stretch ', index === weekData.length - 1 ? '' : 'w-full']"
    >
      <div class="h-full">
        ...
        <div
          :class="[
            'w-8 h-8 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-200 mt-2 mb-8 relative',
            getCircleClasses(dayData, index)
          ]"
        >
          <v-icon
            v-if="dayData.completed >= dayData.total || activeDay === index"
            class="!size-6 sm:size-12"
            :color="dayData.completed >= dayData.total ? '#fff' : '#10a7b0'"
            >mdi-check</v-icon
          >
      ...
      <div
        ...
        :class="[
          ...
          getConnectorClasses(index, weekData[index + 1])
        ]"
      />
    </div>
  </div>
